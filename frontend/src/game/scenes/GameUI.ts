...
import Clock from "@/phaser3-rex-notes/plugins/clock";
...
export default class GameUI extends Phaser.Scene {
    ...
    private addKillCountScore(): void {
        const killCountIcon = this.add.image(0, 0, "icon-skull");
        killCountIcon.setScale(0.6);
    ...
    private addPauseButton(): void {
        const pauseButton = this.add.image(0, 0, "btn-pause").setInteractive();
        pauseButton.setScale(0.5, 0.5);
        ...
        pauseButton.on(
            "pointerover",
            () => {
                selectButton(pauseButton, null, 0.6);
            },
            this
        );
        ...
        pauseButton.on(
            "pointerout",
            () => {
                unselectButton(pauseButton, null, 0.5);
            },
            this
        );
    ...
    private updateExpLevel(exp: number, requiredExp: number): void {
        console.log("updateExpLevel", exp, requiredExp, exp / requiredExp);
    ...
    public update(): void {
        this.gameStartClockText.setText(
            convertMsToTime(this.gameStartClock.now)
        );
    }
}
