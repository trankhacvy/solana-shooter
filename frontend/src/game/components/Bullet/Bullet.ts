...
export default class Bullet extends Phaser.Physics.Arcade.Sprite {
    ...
    public fire1(player: Player, enemy: Phaser.GameObjects.Sprite): void {
        this.body.reset(player.x, player.y);
        ...
        const summarySpeed = 300; //this.getSpeed;

        this.scene.tweens.add({
            targets: this,
            scale: 0.5, //this.getDisplayScale,
            duration: summarySpeed,
            repeat: 0,
        });

        this.rotation = Phaser.Math.Angle.BetweenPoints(player, enemy);

        this.scene.physics.moveTo(this, enemy.x, enemy.y, summarySpeed);
